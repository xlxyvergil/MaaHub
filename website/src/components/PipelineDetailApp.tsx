...
import { ArrowLeft, Download, Star, Clock, GitMerge, Copy, Check, Terminal, FileCode, Tag, ArrowRight } from 'lucide-react';
...
export function PipelineDetailApp({ pipelineId, pipelineData, lang = 'zh' }: { pipelineId: string, pipelineData: any, lang?: 'zh' | 'en' }) {
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(`git clone https://github.com/MaaXYZ/MaaHub.git --depth=1\ncp -r MaaHub/Storage/pipelines/${pipelineData.author}/${pipelineData.id.split('/')[1]} ./your_agent_pipelines/`);
    setIsCopied(true);
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-foreground">{pipelineData.author}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>{pipelineData.downloads || 0}</span>
                  </div>
                </div>
                ...
                <div className="rounded-lg border bg-card p-4 shadow-sm">
                  ...
                  <div className="relative">
                    <pre className="overflow-x-auto rounded bg-muted p-3 text-xs font-mono">
                      <code>MaaHub install pipeline {pipelineData.id}</code>
                    </pre>
                    <button 
                      onClick={handleCopy}
                ...
                <div className="p-6">
                  {activeTab === 'readme' ? (
                    <div className="prose prose-sm dark:prose-invert max-w-none prose-headings:font-bold prose-a:text-primary">
                      ...
                      <h1>{pipelineData.title}</h1>
                      <p>{pipelineData.description}</p>
                      <h2>Overview</h2>
                      ...
                      <pre><code>{`{
  "name": "${pipelineData.title.toLowerCase().replace(/[^a-z0-9]/g, '_')}",
  "version": "1.0.0"
}`}</code></pre>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center p-2 rounded-md hover:bg-muted/50 cursor-pointer border border-transparent hover:border-border transition-colors">
                        <FileCode className="h-4 w-4 mr-3 text-purple-400" />
